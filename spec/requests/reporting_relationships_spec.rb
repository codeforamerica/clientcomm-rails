...
describe 'Reporting Relationship Requests', type: :request, active_job: true do
  ...
  describe 'POST#create' do
    ...
    it 'creates transfer markers' do
      expect(transfer_user.messages.where(type: TransferMarker.to_s)).to be_empty
      time = Time.zone.now
      travel_to time do
        subject
      end
      expect(transfer_user.messages.where(type: TransferMarker.to_s).count).to eq(1)
      marker_from = transfer_user.messages.where(type: TransferMarker.to_s).first
      ...
      expect(user.messages.where(type: TransferMarker.to_s).count).to eq(1)
      marker_to = user.messages.where(type: TransferMarker.to_s).first
